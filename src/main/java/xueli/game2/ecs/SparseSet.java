...
public class SparseSet {
	 ...
	 * @return "True" means we successfully add the element while "False" means we
	 *         find the same element in the set
	 */
	public boolean add(int i) {
		// Because the initial value of array is 0, so we add one to the real data to
		// avoid some problems.
	...
	/**
	 * Get the index of an element. When the value is 0, then this element isn't in
	 * the set. So the index actually starts at 1.
	 * 
	 * @param i
	 ...
	 * @return "True" means we successfully find the element and remove it while
	 *         "False" means we don't find that element
	 */
	...
	public boolean remove(int i) {
		...
		// Remember this index is started from 1. We have to subtract the number by one
		// to get its real index.
		int dataIndex = targetPage[xInSparse];
		if (dataIndex <= 0)
			return false;

		// To save the cost of moving every element backward,
		// First we exchange the position of the element to be removed and the last
		// element.
		// But we don't have to literally set the last element to the element to be
		// removed.
	...
	private boolean setSparse(int index, int data, boolean force) {
		...
		if (targetPage[xInSparse] > 0 && !force)
			return false;
	...
	private int getSparse(int index) {
		int yInSparse = index / sparsePageSize;
		if (yInSparse > sparse.size())
			// Because when creating an array, Java sets all its elements to zero, so we set
			// it to zero
			return 0;
