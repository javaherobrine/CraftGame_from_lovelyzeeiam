...
public class ClockFrame {
	...
	private void initialize() {
		...
		OffsetLayout timeDateOffsetLayout = new OffsetLayout(
				new HorizontalFilledLayout(VerticalAlign.ALIGN_CENTER, HorizontalAlign.CENTER));
...
//		appPanel.add(sysInfoPanel);
		...
		HorizontalFilledLayout backgroundLayout = new HorizontalFilledLayout(VerticalAlign.ALIGN_BOTTOM,
		...
		try {
			backgroundImage = new ImageView(ImageIO.read(
					RESOURCE_PROVIDER.getResource(new Identifier("clock", "images/background.png")).openInputStream()));
		} catch (IOException e) {
		...
		var backgroundImageTransitionCaller = M_TRANSITION_MANAGER
				.registerNewTransition(
						TransitionBindings.newBindingDimension(backgroundImage,
								ValueProvider.newProviderRatioVMin(i -> (int) (i * 0.5), 1.0)),
						Curves.easeOutExpo, 1000);
		var timeFontSizeTransitionCaller = M_TRANSITION_MANAGER
				.registerNewTransition(TransitionBindings.newBindingNumber(lblTime,
						ValueProvider.vminForDouble(10.0, appPanel), PropertyAccessible.fontAccessible(lblTime)));
		var dateFontSizeTransitionCaller = M_TRANSITION_MANAGER
				.registerNewTransition(TransitionBindings.newBindingNumber(lblDate,
						ValueProvider.vminForDouble(3.0, appPanel), PropertyAccessible.fontAccessible(lblDate)));

		var timeDatePanelTransitionValueProvider = ValueProvider.vminForDouble(100.0, null);
		var timeDatePanelSizeTransitionCaller = M_TRANSITION_MANAGER.registerNewTransition(new TransitionBinding() {
...
//		);
		...
		AnimationBinding startAnimBinding = AnimationBindingBuilder.newBuilder()
				.add(new IntValueAnimationBinding(() -> -100, () -> 0, true) {
					@Override
